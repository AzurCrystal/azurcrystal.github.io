...
本文是[网络基建实验](./experiment-on-infrastructure.md)的其中一章。有关其他实验，请参照该文章目录。
...
如果 `P-256` 被攻破了，那么 `P-521` 也将毫无意义，而且效率远低于 `P-256`。
...
本实验采用OpenSSL建立了实验性质的根CA和二级CA，并使用二级CA签发了带有SAN扩展的证书，初步实现了
